...
# Part 1
with open("day19_program.txt") as f:
    content = f.readlines()

program = content[0].strip()
code_list = create_code_list(program)

start_state = ComputerState(0, code_list, halt_on_output=True)
points = 0
for x in range(50):
    for y in range(50):
        new_state = start_state.clone()
        pull = new_state.run(input_entry=[x, y])
        if pull:
            points += 1
            print('#', end='')
        else:
...
print(f"Number of points in beam: {points}")
